...
<form class="form" style="background-color: white; border: 1px solid #e4e7f0; padding: 20px;" role="form" method="POST"
    enctype="multipart/form-data" action="{{route('block.update',$block->id)}}">
    @csrf
    @method('PUT')
    <div class="card-body">
        @if ($errors->any())
            ...
            <div class="form-group row">
                <div class="col-lg-4 col-md-9 col-sm-12">
                    <select class="form-control js-example-basic-single" name="blocker">
                        ...
                        <option value="{{$user->id}}" @if($block->blocker_id == $user->id) selected
            ...
            <div class="form-group row">
                <div class="col-lg-4 col-md-9 col-sm-12">
                    <select class="form-control js-example-basic-single" name="blocked">
                        ...
                        @foreach ($users as $user)
                        <option value="{{$user->id}}" @if($block->blocked_id == $user->id) selected
                            @endif>{{$user->user_name}}</option>
                        @endforeach
                    </select>
                </div>
